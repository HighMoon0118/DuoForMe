...
public interface GoldWinRateRepository extends JpaRepository<GoldWinRate, Long> {
    ...
    @Query(value = "select w.champB, w.winRate from GoldWinRate as w where w.champA = :champName and w.winRate >= 50 order by w.winRate desc")
    List<String> findBestChampBbyChampName(@Param("champName")String champName);

    @Query(value = "select w.champA, w.winRate from GoldWinRate as w where w.champB = :champName and w.winRate >= 50 order by w.winRate desc")
